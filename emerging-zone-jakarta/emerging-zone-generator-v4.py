...
/* ============ Page 5: Detail Kawasan 2x2+1 grid (all 5 indikator visible) ============ */
.page5-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
.page5-grid .p5-full { grid-column: 1 / -1; }
@media (max-width: 768px) { .page5-grid { grid-template-columns: 1fr; } }
...
<section x-show="page === 'detail-kawasan'">
  ...
  <div class="chart-card mt-8">
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
      ...
      <select
        @change="const v=$event.target.value; const [m,...rest]=v.split('::'); page5Mode=m; page5Selected=rest.join('::');"
  ...
  <!-- All 5 indicators in 2x2+1 grid (single section, no sub-tab) -->
  <div class="page5-grid mt-6">
    <div id="p5-ntl"       class="chart-card"></div>
    <div id="p5-podes"     class="chart-card"></div>
    <div id="p5-sektor"    class="chart-card"></div>
    <div id="p5-fasilitas" class="chart-card"></div>
    <div id="p5-akses"     class="chart-card p5-full"></div>
  </div>
</section>
"""
...
function renderPage5(ctx) {
  ...
  `;
  renderPage5AllIndicators(ctx);
}
// Render all 5 indikator at once into 2x2+1 grid (no sub-tab anymore)
function renderPage5AllIndicators(ctx) {
  const item = getPage5Item(ctx);
  if (!item) return;
  const isKaw = ctx.page5Mode !== 'desa';

  // 1. Luminositas Malam
  const lum2021 = item.lum_2021;
  const lum2024 = item.lum_2024;
  const lum2025 = item.lum_2025;
  const ntlCagr = item.ntl_cagr_2021_2025;
  const qual = item.quality_flag;
  const hasLumTrend = (lum2021 != null) && (lum2024 != null) && (lum2025 != null);
  document.getElementById('p5-ntl').innerHTML = `
    ...
    ${hasLumTrend ? `<div id="p5-ntl-chart" style="height:220px;"></div>` : ''}
    <div style="font-size:13px;color:var(--muted);margin-top:14px;line-height:1.6;">
      Pertumbuhan tahunan rata-rata: <strong style="color:var(--ink);">${fmtPct(ntlCagr, ' per tahun')}</strong>
      ${qual ? `<br>Quality flag: <em>${qual}</em>` : ''}
    </div>
    <div style="margin-top:10px;">
  ...
  // 2. Tempat Usaha (Podes)
  const cnt = item.podes_count_2025;
  const podesCagr = item.podes_cagr_2021_2025;
  const divShannon = item.podes_diversity_shannon;
  const flag = item.podes_winsorize_flag;
  document.getElementById('p5-podes').innerHTML = `
    ...
    <div id="p5-podes-chart" style="height:220px;"></div>
    <div style="font-size:13px;color:var(--ink);margin-top:14px;line-height:1.7;">
      ...
      Pertumbuhan tahunan rata-rata: <strong>${fmtPct(podesCagr, ' per tahun')}</strong><br>
      Indeks keberagaman (Shannon): <strong>${fmtScore(divShannon)}</strong>
      ${flag ? '<br><span style="color:var(--warning);">&#9888; Data podes diwinsorize.</span>' : ''}
    </div>
  `;
  Plotly.newPlot('p5-podes-chart', [{
    type: 'bar',
    x: ['Tempat Usaha 2025'],
    y: [cnt],
    text: [fmtNum(cnt)],
    ...
    annotations: [{ x: 0, y: cnt, text: fmtPct(podesCagr, ' per tahun'), showarrow: false, yshift: 30, font: { size: 13, color: PLOT_COLORS.positive } }]
  }), PLOT_CFG);

  // 3. Sektor Usaha
  const perd = item.magnitude_perdagangan_2025;
  const akm  = item.magnitude_akomamin_2025;
  const mag  = item.magnitude_2025;
  const elSektor = document.getElementById('p5-sektor');
  if (perd != null && akm != null) {
    elSektor.innerHTML = `
      ...
      <div id="p5-sektor-chart" style="height:240px;"></div>
    ...
    `;
    Plotly.newPlot('p5-sektor-chart', [{
      type: 'pie', hole: 0.55,
    ...
    elSektor.innerHTML = `
      <div class="eyebrow" style="color:var(--muted);">Sektor Usaha</div>
      <h3 class="serif-display text-xl mt-1 mb-3">Indeks Aktivitas Komersial</h3>
      <div style="margin-top:18px;border-left:3px solid var(--navy);padding:8px 16px;background:linear-gradient(90deg,rgba(0,61,121,0.06),transparent);">
        <div class="eyebrow" style="color:var(--muted);">Total Indeks Wilayah</div>
        <div class="stat-num" style="font-size:34px;color:var(--navy);font-family:'Source Serif 4',Georgia,serif;font-weight:600;margin-top:4px;">${fmtRupiah(mag)}</div>
      </div>
      <div style="font-size:11px;color:var(--muted);margin-top:14px;font-style:italic;">Indeks proxy: jumlah usaha &times; rata-rata pendapatan pekerja sektor (Sakernas). Bukan klaim Rupiah absolut.</div>
    `;
  }
  ...
  document.getElementById('p5-fasilitas').innerHTML = `
    ...
    <div id="p5-fasil-chart" style="height:260px;"></div>
  `;
  Plotly.newPlot('p5-fasil-chart', [{
    type: 'bar', orientation: 'h',
  ...
  // 5. Aksesibilitas (full width row)
  const trans = item.transport_access_score_raw;
  const catch_ = item.catchment_density_1km;
  document.getElementById('p5-akses').innerHTML = `
    <div class="eyebrow" style="color:var(--muted);">Aksesibilitas Transportasi</div>
    <h3 class="serif-display text-xl mt-1 mb-3">Akses Transportasi dan Densitas Catchment</h3>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div style="border-top:3px solid var(--navy);padding-top:14px;">
        <div class="eyebrow" style="color:var(--muted);">Skor Akses Transportasi</div>
        <div style="font-family:'Source Serif 4',Georgia,serif;font-size:34px;font-weight:600;color:var(--navy);margin-top:4px;">${fmtScore(trans)}</div>
        <div style="font-size:12px;color:var(--muted);margin-top:6px;">Komposit halte/stasiun dalam radius 1km, weighted by mode</div>
      </div>
      <div style="border-top:3px solid var(--yellow);padding-top:14px;">
        <div class="eyebrow" style="color:var(--muted);">Densitas Catchment 1km</div>
        <div style="font-family:'Source Serif 4',Georgia,serif;font-size:34px;font-weight:600;color:var(--ink);margin-top:4px;">${fmtNum(catch_)}</div>
        <div style="font-size:12px;color:var(--muted);margin-top:6px;">Jumlah node transportasi dalam radius 1km dari centroid</div>
      </div>
    </div>
  `;
}
...
function dashboard() {
  return {
    ...
    init() {
      ...
      // Page 5 dropdown: re-render setiap kali user pilih wilayah/desa baru
      this.$watch('page5Selected', () => this.$nextTick(() => { if (this.page === 'detail-kawasan') renderPage5(this); }));
      this.$watch('page5Mode', () => this.$nextTick(() => { if (this.page === 'detail-kawasan') renderPage5(this); }));
...
window.renderPage5AllIndicators = renderPage5AllIndicators;
