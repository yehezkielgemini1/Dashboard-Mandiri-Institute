...
<div class="hero-band">
  <header class="max-w-[1280px] mx-auto px-8 pt-14 pb-14">
    ...
    <h1 class="serif-display text-4xl md:text-5xl mt-5 max-w-4xl">Pendeteksi Kawasan Komersial Berkembang &mdash; DKI Jakarta 2021-2025</h1>
    <p class="mt-5 text-base max-w-3xl leading-relaxed">Identifikasi 21 wilayah berpotensi dan 270 kelurahan dengan sinyal pertumbuhan komersial terkuat 2021-2025, melalui composite Cahaya Malam (NASA VIIRS) + Sensus Tempat Usaha (Podes BPS) + Pendapatan Pekerja Sektor (Sakernas BPS) + Pemetaan Tempat Usaha Gaya Hidup (OpenStreetMap).</p>
  </header>
</div>
"""
...
<section x-show="page === 'ringkasan'">
  <div class="rule-top pt-8">
    ...
    <h2 class="serif-display text-3xl mt-2 max-w-3xl">Wilayah komersial yang sedang menyalakan diri di Jakarta.</h2>
    <p class="mt-3 muted text-sm max-w-3xl">Composite empat sumber data mengidentifikasi 21 wilayah berpotensi dan memberi peringkat 270 kelurahan menurut kekuatan pertumbuhan komersial 2021-2025. Skor Potensi mengukur sinyal pertumbuhan tahunan, sementara Skor Magnitude mengukur skala aktivitas saat ini.</p>
  </div>
  ...
  <div class="mt-10 chart-card">
    <div class="eyebrow" style="color:var(--muted)">Tren Cahaya Malam 2019-2025</div>
...
<section x-show="page === 'cahaya-malam'">
  <div class="rule-top pt-8">
    <span class="badge"><span class="badge-dot"></span>Cahaya Malam (Satelit)</span>
    <div class="eyebrow-roman mt-3">II. Sinyal dari Atas Atap</div>
    <h2 class="serif-display text-3xl mt-2 max-w-3xl">Pertumbuhan cahaya malam per kelurahan, 2021-2025.</h2>
    <p class="mt-3 muted text-sm max-w-3xl">Cahaya malam dari satelit NASA VIIRS adalah proxy paling konsisten untuk intensitas aktivitas spasial. Pertumbuhan tahunan rata-rata menunjukkan kelurahan mana yang sedang menambah aktivitas, sementara tingkat kecerahan 2025 menunjukkan skala aktivitas saat ini.</p>
  </div>
  ...
  <div class="chart-card mt-8">
    ...
    <h3 class="serif-display text-xl mt-1 mb-4">Lintasan Cahaya Malam per Wilayah</h3>
  ...
  <div class="chart-card mt-8">
    ...
    <div class="chart-footer">
      <span><span class="label">Indikator</span>Perubahan tahunan cahaya malam awal 2026 vs awal 2025</span>
    </div>
  </div>
</section>
"""
...
<section x-show="page === 'aktivitas-komersial'">
  <div class="rule-top pt-8">
    ...
    <h2 class="serif-display text-3xl mt-2 max-w-3xl">Kepadatan dan keberagaman tempat usaha per kelurahan.</h2>
    <p class="mt-3 muted text-sm max-w-3xl">Sensus Podes BPS adalah rujukan jumlah tempat usaha (10 kategori), sementara OpenStreetMap memberikan titik gaya hidup, hiburan, dan layanan untuk visualisasi. Heatmap dan cluster marker membantu melihat klaster usaha gaya hidup yang biasanya di luar cakupan sensus formal.</p>
  </div>
...
<section x-show="page === 'skor-potensi'">
  <div class="rule-top pt-8">
    ...
    <h2 class="serif-display text-3xl mt-2 max-w-3xl">Dua perspektif: pertumbuhan dan kepadatan.</h2>
    <p class="mt-3 muted text-sm max-w-3xl">Skor Potensi mengukur sinyal pertumbuhan tahunan (siapa yang sedang naik). Skor Magnitude mengukur skala aktivitas 2025 (siapa yang sudah besar). Kedua skor independen dan saling melengkapi.</p>
  </div>
  ...
  <div x-show="page4Tab==='eczi'">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="chart-card">
        ...
        <h3 class="serif-display text-xl mt-1 mb-3">Pertumbuhan Cahaya Malam vs Gaya Hidup</h3>
...
<section x-show="page === 'detail-kawasan'">
  <div class="rule-top pt-8">
    ...
    <h2 class="serif-display text-3xl mt-2 max-w-3xl">Profil per wilayah berpotensi atau kelurahan.</h2>
    <p class="mt-3 muted text-sm max-w-3xl">Pilih satu wilayah berpotensi (21) atau satu kelurahan (270) untuk melihat semua komponen skor secara mendalam.</p>
  </div>
    ...
    <div style="display:flex;align-items:center;gap:24px;flex-wrap:wrap;">
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer;">
          <input type="radio" x-model="page5Mode" value="kawasan" @change="page5Selected = KAWASAN_DATA[0].kawasan; $nextTick(()=>renderPage5(this))"> Wilayah Berpotensi
        </label>
        <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer;">
          <input type="radio" x-model="page5Mode" value="desa" @change="page5Selected = DESA_DATA[0].nama_desa; $nextTick(()=>renderPage5(this))"> Kelurahan/Desa
        </label>
      </div>
      <div style="display:flex;align-items:center;gap:12px;flex:1;">
        <span class="filter-bar-label">Pilih</span>
        <select x-model="page5Selected" @change="$nextTick(()=>renderPage5(this))" class="select-flat" style="font-size:14px;min-width:280px;">
          <template x-if="page5Mode==='kawasan'">
            <template x-for="k in KAWASAN_DATA.map(x=>x.kawasan).sort()" :key="k">
              <option :value="k" x-text="k"></option>
            </template>
          </template>
          <template x-if="page5Mode==='desa'">
            <template x-for="d in DESA_DATA.map(x=>x.nama_desa).sort()" :key="d">
  ...
  <div class="tab-strip mt-6">
    <button @click="page5SubTab='ntl'; $nextTick(()=>renderPage5Sub(this))" :class="page5SubTab==='ntl' ? 'tab-btn active' : 'tab-btn'">Cahaya Malam</button>
...
<section x-show="page === 'metodologi'">
  <div class="rule-top pt-8">
    ...
    <h2 class="serif-display text-3xl mt-2 max-w-3xl">Bagaimana skor dihitung dan apa batasannya.</h2>
    <p class="mt-3 muted text-sm max-w-3xl">Dashboard ini menggabungkan 4 sumber data publik: cahaya malam NASA VIIRS, sensus tempat usaha Podes BPS, pendapatan pekerja sektor Sakernas BPS, dan titik tempat usaha OpenStreetMap. Bobot komposit divalidasi terhadap rujukan empiris.</p>
  </div>
  ...
  <div class="chart-card">
    <table class="score-table">
      ...
      <tbody>
        ...
        <tr><td><strong>Tren 2026</strong></td><td>Sinyal awal 2026 dari cahaya malam</td><td>Lanjut Tumbuh, Stabil, Mulai Melambat &mdash; berdasarkan perubahan tahunan awal 2026 vs awal 2025.</td></tr>
      </tbody>
    </table>
  </div>
  ...
  <div class="chart-card">
    <table class="score-table">
      ...
      <tbody>
        <tr><td>NASA VIIRS Cahaya Malam</td><td>Intensitas aktivitas spasial</td><td>270 kelurahan (173 nilai unik via spatial join)</td><td>2019-2025</td></tr>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    ...
    <div class="chart-card">
      <table class="score-table">
        ...
        <tbody>
          <tr><td rowspan="3"><strong>Cahaya Malam (24%)</strong></td><td>Pertumbuhan tahunan rata-rata</td><td class="num">13%</td></tr>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    <div class="chart-card">
      ...
      <h4 class="serif-display text-lg mt-1 mb-3">Cahaya Malam vs Indeks Komersial</h4>
      <table class="score-table">
      ...
      <p style="font-size:13px;margin-top:12px;color:var(--ink);line-height:1.5;">Cahaya malam dan Indeks Komersial mengukur dimensi yang berbeda. Keduanya wajib dipertahankan dalam composite &mdash; tidak bisa saling menggantikan.</p>
    </div>
    <div class="chart-card">
  ...
  <div class="chart-card">
    <ol style="margin-left:20px;font-size:13px;line-height:1.7;color:var(--ink);">
      <li>Cahaya malam adalah proxy aktivitas spasial, bukan ukuran langsung PDRB atau transaksi.</li>
      ...
      <li>Tren 2026 = perubahan tahunan cahaya malam awal 2026 vs awal 2025; sinyal awal, bukan kesimpulan tahun penuh.</li>
  ...
  <div class="chart-card">
    <table class="score-table">
      ...
      <tbody>
        <tr><td>Cahaya Malam VIIRS</td><td>Bulanan (lag 2-4 minggu)</td><td>Refresh indikator Tren 2026</td></tr>
...
function renderMap3() {
  initMap3();
  if (_map3Layer) {
    _map3.removeLayer(_map3Layer); _map3Layer = null;
  }
  const ctx = document.querySelector('[x-data]').__x.$data;
  const layer = ctx.page3Layer;
  if (layer === 'podes') {
...
function renderPage4Eczi(ctx) {
  ...
  const cats = ['Cahaya Malam','Tempat Usaha','Aktivitas Komersial','Gaya Hidup','Aksesibilitas'];
  ...
  Plotly.newPlot('chart-quadrant', [{
    ...
    xaxis: { title: 'Skor Pertumbuhan Cahaya Malam (0-100)', gridcolor: '#E5E8EC', range: [0, 100] },
    ...
    annotations: [
      ...
      { x: 95, y: 5, text: 'Pertumbuhan<br>Cahaya Tinggi', showarrow: false, font: { size: 9, color: '#667085' }, align: 'right' },
  ...
  html += '<th class="num">Skor Potensi</th><th class="num">Cahaya Malam</th><th class="num">Tempat Usaha</th><th class="num">Aktivitas Komersial</th><th class="num">Gaya Hidup</th><th class="num">Aksesibilitas</th><th>Kategori</th><th>Tren 2026</th></tr></thead><tbody>';
  sorted.forEach((d, i) => {
    let g1, g2, g3, g4, g5;
...
function renderPage5Sub(ctx) {
  ...
  if (sub === 'ntl') {
    ...
    c.innerHTML = `
      <div class="eyebrow" style="color:var(--muted);">Cahaya Malam</div>
...
function renderMetodologi(ctx) {
  ...
  Plotly.newPlot('chart-weights', [{
    type: 'pie', hole: 0.45,
    labels: ['Cahaya Malam (24%)', 'Tempat Usaha (33%)', 'Aktivitas Komersial (11%)', 'Gaya Hidup (12%)', 'Aksesibilitas (20%)'],
  ...
  // References
  const refs = [
    { id: 'R1', cit: 'Henderson, J. V., Storeygard, A., & Weil, D. N. (2012). Measuring Economic Growth from Outer Space. American Economic Review, 102(2), 994-1028.', doi: '10.1257/aer.102.2.994', finding: 'Cahaya malam adalah proxy konsisten untuk pertumbuhan ekonomi sub-nasional.' },
    ...
    { id: 'R3', cit: 'Bluhm, R., & Krause, M. (2022). Top lights: Bright cities and their contribution to economic development. Journal of Development Economics, 157.', doi: '10.1016/j.jdeveco.2022.102880', finding: 'Saturasi cahaya malam di kota besar; penting koreksi BRDF.' },
...
function dashboard() {
  return {
    ...
    tabs: [
      ...
      { id: 'cahaya-malam', label: 'Cahaya Malam', icon: 'mdi:weather-night' },
